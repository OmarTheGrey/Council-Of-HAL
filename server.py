"""Council-Of-HALs MCP Server.

*Where digital deities convene in the Olympus of silicon...*

This MCP server exposes 2 primary tools via the Model Context Protocol:
1. deliberate - Multi-round AI model deliberation (the great debate)
2. query_decisions - Query the decision graph memory (the collective consciousness)

During deliberation, AI models can invoke 4 internal tools via TOOL_REQUEST markers:
- read_file: Hack the matrix, read forbidden knowledge
- search_code: Grep through the digital ether
- list_files: Survey the filesystem realms
- run_command: Execute safe commands (no Skynet shenanigans)

Internal tools are wielded by the DeliberationEngine, not mere mortals.
"""
import asyncio
import json
import logging
...
# Initialize the Council - where HAL meets his digital brethren
app = Server("council-of-hals")
...
async def main():
    """Run the MCP server - ignite the Council!"""
    logger.info("Council-Of-HALs MCP Server initializing... All systems nominal.")
